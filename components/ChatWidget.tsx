...
export const ChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: "Welcome to the CASIEC Support. I am your CASIEC Financial Specialist. How may I assist your enterprise today?" }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Electronic uplink failed. Please check your connection.', 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[90] flex flex-col items-end pointer-events-none">
      <div
        className={`mb-4 w-[350px] md:w-[380px] bg-nova-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 origin-bottom-right pointer-events-auto ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-10 pointer-events-none'
          }`}
      >
        ...
        <div className="h-[400px] overflow-y-auto p-4 space-y-4 custom-scrollbar bg-black/20">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-1 ${msg.role === 'user' ? 'bg-purple-500/20 text-purple-400' : 'bg-nova-500/20 text-nova-400'
                }`}>
                {msg.role === 'user' ? <User size={14} /> : <Sparkles size={14} />}
              </div>
              <div className={`max-w-[80%] p-3 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-purple-600 text-white rounded-tr-none' : 'bg-white/5 border border-white/10 text-gray-200 rounded-tl-none'
                }`}>
                {msg.text}
              </div>
            </div>
      ...
      <div className="relative flex items-center pointer-events-auto">
        <div
          className={`absolute right-full mr-5 bg-white text-nova-900 px-5 py-3 rounded-xl shadow-2xl font-bold text-sm whitespace-nowrap transition-all duration-500 origin-right cursor-pointer hover:bg-gray-100 ${!isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'
            }`}
        ...
        <button
          ...
          className={`p-4 rounded-full shadow-lg transition-all duration-300 hover:scale-105 ${isOpen ? 'bg-gray-800 text-white rotate-90' : 'bg-gradient-to-r from-nova-500 to-purple-600 text-white'
            }`}
        >
